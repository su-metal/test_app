...
要件準拠方針（重要）
- 正式要件は「フードロス削減アプリ_要件・仕様書_v_2_（2025-09-26）」に従います（以降「要件書」）。
...
- ステータスや文言は要件書の表記に寄せ、既存UIの日本語表現と齟齬が出ないよう統一します。
...
- 在庫: 店側UI操作と簡易同期で運用。外部在庫システム連携は未実装。
...
- apps/user
  ...
  - 店舗・商品取得: `stores`（全件）と `products` を読み込み、`store_id` でグルーピングして `shops` を構築（テスト段階では店舗はDB由来、商品の全店対応は段階的に移行）。
...
- Phase 1（要件書準拠の拡張）
  ...
  - 実装順は要件書の優先度に従い、DBスキーマ変更はマイグレーション前提で計画。
...
注意
- 本記述は現行コードの参照カラムに基づくものです。提供された「データベース構成（supabase）_2025_10_10.md」と差分がある場合は、スキーマの正を上流（ドキュメント）とし、次の優先度で修正してください。
  1) 互換範囲ならアプリ側の参照名をドキュメントに合わせる。
  2) 互換不可の場合は移行（追加カラム/ビュー）で吸収し、段階的に参照を切替。
  3) 差分と計画を `TODO(req v2)` でコードに最小限メモ。
